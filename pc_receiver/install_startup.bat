@echo off
rem Legacy/manual entry point - prefer Install_PhoneCam.vbs (in the project
...
echo Adding PhoneCam to Windows startup...

set "SHORTCUT=%APPDATA%\Microsoft\Windows\Start Menu\Programs\Startup\PhoneCam.lnk"
set "TARGET=wscript.exe"
set "ARGS=""%~dp0PhoneCam_Service.vbs"""
...
if %ERRORLEVEL% equ 0 (
    echo.
    echo Done! PhoneCam_Service.vbs now registers at login, but it stays
